...
class CIFWrapper(object):
    ...
    def unwrap(self):
        ...
        cleaned_map = collections.OrderedDict()
        for k, v in list(self._DATA.items()):
            cleaned_map.setdefault(k, collections.OrderedDict())
...
class Category(object):
    ...
    def __init__(self, category_id, parent):
        ...
        self.recycleBin = collections.OrderedDict()
        self.isTable = False
...
class DataBlock(object):
    ...
    def __init__(self, block_id, parent):
        ...
        self.id = block_id
        self.categories = collections.OrderedDict()
        self.saveFrames = collections.OrderedDict()
        self.recycleBin = collections.OrderedDict()
...
class CifFile(object):
    ...
    def __init__(self, file_path=None, mmcif_data_map=None):
        """"""
        self.data_blocks = collections.OrderedDict()
        self.recycleBin = collections.OrderedDict()
...
def _formatVal(val):
    ...
    if "'" in val:
        ...
        val = ('"%s"' % val) if val.startswith("_") \
            or val.startswith("[") \
            or (" " in val and "\n" not in val and not val.startswith(' ')) else val
    if "\n" in val:
        if val[0] in ["'", '"']:
            val = val[1:-1]
        val = "\n;" + val + "\n;\n"
