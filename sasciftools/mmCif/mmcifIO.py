        ...
        for datablock in cifObjIn.getDataBlocks():
            ...
            for category in datablock.getCategories():
                ...
                for item in category.getItems():
                    ...
                    if not item.isColumn:
                        if table:
                            entryLen = []
                            entryNum = []
                            for col in table:
                                entryLen.append(len(max(col,key=len)))
                                colNum = all(self.is_number(i) for i in col)
                                entryNum.append(colNum)
                            if any(cell.startswith('\n;') for cell in col for col in table):
                                for rI in range(len(table[0])):
                                    row_items = [table[i][rI] for i in range(len(table))]
                                    line = " ".join(row_items) + "\n"
                                ...
                                for rI in range(len(table[0])):
                                    ...
                                    for i in range(len(table)):
                                        if entryNum[i]:
                                            row_items.append(table[i][rI].rjust(entryLen[i]))
                                        else:
...
class CifFileReader(object):
    ...
    def read(self, file_path, output='cif_dictionary', ignore=[]):
        if self.input == 'data':
            ...
            mmcif_dict = collections.OrderedDict()
...
class MMCIF2Dict():
    ...
    def _parseFile(self, file_path, ignoreCategories):
        ...
        mmcif_like_file = collections.OrderedDict()
        data_heading = ""
        data_block = collections.OrderedDict()
        save_block = collections.OrderedDict()
        line_num = 0
        try:
                    ...
                    if self.dataRE.match(line):
                        ...
                        if table_values_array != []:
                            for itemIndex, name in enumerate(table_names):
                                data_block[category].update(collections.OrderedDict({name:[row[itemIndex] for row in table_values_array]}))
                            table_values_array = []
                        isLoop = True
                        ...
                        if not isLoop:
                            ...
                            if ignoreCategories and category in ignoreCategories:
                                ...
                                if category in data_block:
                                    data_block[category].update(collections.OrderedDict({item: value if len(value) > 1 else value[0]}))
                                else:
                                    data_block.setdefault(category, collections.OrderedDict({item: value if len(value) > 1 else value[0]}))
                        else:
                            if ignoreCategories and category in ignoreCategories:
                                skipCategory = True
                            else:
                                data_block.setdefault(category, collections.OrderedDict())
